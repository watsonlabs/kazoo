...
handle(Data, Call) ->
    {'ok', DirJObj} = couch_mgr:open_doc(whapps_call:account_db(Call)
                                         ,wh_json:get_value(<<"id">>, Data)
                                        ),
    whapps_call_command:answer(Call),
    case get_directory_listing(whapps_call:account_db(Call)
                               ,wh_json:get_value(<<"_id">>, DirJObj)
                              )
    of
        {'ok', Users} ->
            State =
                #directory{
                   sort_by = get_sort_by(wh_json:get_value(<<"sort_by">>, DirJObj))
...
directory_start(Call, State, CurrUsers) ->
    ...
    collect_min_digits(Call, add_dtmf(clear_dtmf(State), DTMF)
                       ,CurrUsers, min_dtmf(State) - byte_size(DTMF)
                      ).

-spec collect_min_digits(whapps_call:call(), directory(), directory_users(), integer()) ->
                                'ok'.
...
-spec maybe_match_user(whapps_call:call(), directory_user(), pos_integer()) ->
                              'route' |
                              'next' |
                              'start_over' |
                              'invalid' |
...
%%------------------------------------------------------------------------------
-spec callflow(whapps_call:call(), directory_user()) ->
                      wh_json:object() | 'fail'.
...
get_directory_listing(Db, DirId) ->
    case couch_mgr:get_results(Db, ?DIR_DOCS_VIEW, [{'key', DirId}
                                                    ,'include_docs'
                                                   ])
    of
        {'ok', []} ->
            ...
            {'error', 'no_users_in_directory'};
        {'ok', Users} ->
            {'ok', [ get_directory_user(wh_json:get_value(<<"doc">>, U)
                                        ,wh_json:get_value(<<"value">>, U)
                                       ) || U <- Users]};
        {'error', _E}=E ->
...
log(Users) ->
    [lager:info("user: ~s: ~s: ~s", [full_name(U)
                                     ,last_first_dtmfs(U)
                                     ,first_last_dtmfs(U)
                                    ])
     || U <- Users
