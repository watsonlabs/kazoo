...
%%% @doc
%%% Data:
%%%   id: document ID of the voicemail box
%%%   action:
...
-type message_menu_error() :: 'channel_hungup' | 'channel_unbridge' | wh_json:object().

-spec message_menu(mailbox(), whapps_call:call()) ->
                          {'error', message_menu_error()} |
...
%% @end
%%--------------------------------------------------------------------
-spec set_folder(ne_binary(), wh_json:object(), mailbox(), whapps_call:call()) ->
                        'false' | update_folder_return().
...
%% @end
%%--------------------------------------------------------------------
-type update_folder_return() :: {'ok', wh_json:object()} |
                                {'error', term()}.
-spec update_folder(ne_binary(), api_binary(), mailbox(), whapps_call:call()) ->
                           update_folder_return().
    ...
    case couch_mgr:open_doc(AccountDb, Id) of
        {'ok', JObj} ->
            ...
            case couch_mgr:save_doc(AccountDb, wh_json:set_value(<<"messages">>, Messages, JObj)) of
                {'error', 'conflict'} -> update_folder(Folder, MediaId, Mailbox, Call);
                {'ok', _}=OK -> OK;
                {'error', R}=E ->
